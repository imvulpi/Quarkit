...
static inline int 
bsr32(uint32_t value) {
    if (value == 0) return -1;
    // This is a compiler intrinsic, not a function call to a library
    return 31 - __builtin_clz(value);
}

/**
 * Returns the index of the least significant bit set.
 * Example: 0x8 (binary 1000) returns 3.
 */
int ctz32(uint32_t value) {
    if (value == 0) return -1; // Or 32, depending on your architecture
